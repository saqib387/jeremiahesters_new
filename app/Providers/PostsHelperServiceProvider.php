...
class PostsHelperServiceProvider extends ServiceProvider
{
    ...
    /**
     * Get user by it's username (or name slug).
     * Tries exact match, then case-insensitive username, then slug from name.
     *
     * @param $username
     * @return mixed
     */
    public static function getUserByUsername($username)
    {
        ...
        // 2. Case-insensitive username (so /johndoe matches "JohnDoe" in DB)
        $user = User::whereRaw('LOWER(username) = ?', [strtolower($username)])->first();
        if ($user) {
            return $user;
        }

        // 3. Fallback: match by name slug (when username not set or URL uses name-based handle)
        $slug = strtolower(preg_replace('/\s+/', '', $username));
        if ($slug !== '') {
            $user = User::whereRaw("LOWER(REPLACE(REPLACE(name, ' ', ''), '\t', '')) = ?", [$slug])->first();
            if ($user) {
                return $user;
            }
        }
