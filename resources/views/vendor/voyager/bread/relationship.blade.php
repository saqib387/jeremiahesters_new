        ...
        @if($options->type == 'belongsTo')
            ...
            @if(isset($view) && ($view == 'browse' || $view == 'read'))
                ...
                @php
                    ...
                    $fk = $relationshipData->{$options->column} ?? null;
                    $fkEmpty = ($fk === null || $fk === '' || $fk === 0 || $fk === '0');
                    $query = null;
                    if (!$fkEmpty) {
                        $query = $model::where($options->key, $fk)->first();
                    }
                @endphp
            ...
            @else
                ...
                >
                    @php
                        $model = app($options->model);
                        $selectedId = old($options->column, $dataTypeContent->{$options->column});
                        $query = $model::query()->orderBy($options->label)->get();
                    @endphp
                    ...
                    @foreach($query as $relationshipData)
                        <option value="{{ $relationshipData->{$options->key} }}" @if($selectedId == $relationshipData->{$options->key}) selected="selected" @endif>{{ $relationshipData->{$options->label} }}</option>
                    @endforeach
                </select>
                    ...
                    <div>
                        <div class="identity-files-preview">
                            @foreach($query as $query_res)
                                ...
                                @else
                                    <div>{{ $query_res->{$options->label} }}</div>
                                @endif
                            @endforeach
                        </div>
                    </div>
