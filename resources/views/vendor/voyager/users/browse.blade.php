...
@section('content')
    <div class="page-content browse container-fluid jf-dash-page jf-users-page">
        @include('voyager::alerts')
        <div class="row">
            <div class="col-md-12">
                <div class="panel panel-bordered jf-dash-card jf-dash-card--users-table users-table-panel">
                    <div class="panel-body jf-dash-card__body users-table-panel-body">
                        @if ($isServerSide)
                        ...
                        <div class="table-responsive users-table-responsive">
                            <table id="dataTable" class="table table-hover jf-tokens-table jf-users-table">
                                <thead>
                                    <tr>
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        @foreach($dataType->browseRows as $row)
                                            ...
                                            <td>
                                                ...
                                                @elseif($row->type == 'image')
                                                    <img src="@if( !filter_var($data->{$row->field}, FILTER_VALIDATE_URL)){{ $dataType->slug == 'users' ? getUserAvatarAttribute( $data->{$row->field} ) : Voyager::image( $data->{$row->field} ) }}@else{{ $data->{$row->field} }}@endif" class="jf-users-avatar w-100p">
                                        ...
                                        @endforeach
                                        <td class="no-sort no-click bread-actions jf-users-actions">
                                            @foreach($actions as $action)
                                                @if (!method_exists($action, 'massAction'))
    ...
    {{-- Single delete modal --}}
    <div class="modal modal-danger fade jf-wallets-modal jf-delete-confirm-modal jf-users-delete-modal" tabindex="-1" id="delete_modal" role="dialog">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <form action="#" id="delete_form" method="POST" class="jf-delete-confirm-modal__form">
                        {{ method_field('DELETE') }}
                        {{ csrf_field() }}
